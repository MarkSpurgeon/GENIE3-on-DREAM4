...
genie3.out <- GENIE3(expr.matrix=dream4.expr.matrix)

link.list.out <- get.link.list(genie3.out)

interaction.list.orig[[data.idx]] <- data.frame(sapply(link.list.out[,1],function(x) strsplit(x,split="G")[[1]][2]),
                                                sapply(link.list.out[,2],function(x) strsplit(x,split="G")[[1]][2]),
                                                link.list.out[,3])
}

save(file="genie3_orig_out.RData",interaction.list.orig)